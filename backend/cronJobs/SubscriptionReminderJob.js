...
// Cron chạy lúc 9h sáng mỗi ngày
cron.schedule('0 9 * * *', async () => {
  try {
    console.log(`📬 [Cron] Kiểm tra gói sắp hết hạn (${DateTime.now().toFormat('yyyy-MM-dd HH:mm')})`);

    const now = DateTime.now().setZone('Asia/Ho_Chi_Minh');
    const reminderThreshold = now.plus({ days: 2 }); // còn 2 ngày nữa là hết

    // Tìm subscription còn hoạt động và sẽ hết hạn sau đúng 2 ngày
    const upcomingExpirations = await Subscription.find({
      IsActive: true,
      EndDate: {
        $gte: now.startOf('day').toJSDate(),
        $lte: reminderThreshold.endOf('day').toJSDate()
      },
      ReminderSent: { $ne: true } // đảm bảo không gửi lại nhiều lần
    }).populate('User Plan');
